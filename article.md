# From Zero Creds to Enterprise Admin: An Active Directory Attack Chain

**Published:** May 20, 2025
**Author:** Anubhav Gain
**Category:** Active Directory Security, Penetration Testing
...
[*] [NTLM] NTLMv2-SSP Hash     : FILE-SERVER01$::CORP:1122334455667788:A1B2C3D4E5F6...

[*] [NTLM] NTLMv2-SSP Client   : 192.168.0.64
[*] [NTLM] NTLMv2-SSP Username : CORP\jifko.admin
[*] [NTLM] NTLMv2-SSP Hash     : jifko.admin::CORP:1122334455667788:A1B2C3D4E5F6...

[*] [NTLM] NTLMv2-SSP Client   : 192.168.0.28
[*] [NTLM] NTLMv2-SSP Username : CORP\sqlservice
[*] [NTLM] NTLMv2-SSP Hash     : sqlservice::CORP:1122334455667788:A1B2C3D4E5F6...
```
...
Unlike high-level libraries that abstract away the protocol details, Impacket gives you direct access to the raw protocol interactions, making it perfect for offensive security testing. My attack chain relied primarily on two Impacket tools:

- **ntlmrelayx** - For relaying NTLM authentication
- **secretsdump** - For extracting credentials via DCSync

Other Impacket gems include:
- `psexec`, `wmiexec`, and `smbexec` for remote command execution
- `GetUserSPNs` for Kerberoasting
- `GetNPUsers` for AS-REP roasting

The framework handles all the low-level details of Windows authentication, letting you focus on the attack strategy rather than protocol implementation.
...
2. **It bypasses common defenses** - Credential guard or Protected Users groups don't stop it
3. **It extracts ALL credentials** - Including service accounts and privileged users
4. **It doesn't require direct access** - No need to touch the NTDS.dit file
5. **It can be performed remotely** - Works over the network
6. **It leaves minimal traces** - Fewer event logs compared to other extraction methods
...
CORP\user1:1126:aad3b435b51404eeaad3b435b51404ee:<REDACTED>:::
[MANY MORE ACCOUNTS]
```
...
Evil-WinRM leverages Windows Remote Management to give you a remote shell on Windows systems. It runs over standard ports often allowed through firewalls (5985/5986), provides a fully interactive PowerShell session, and includes convenient features for file transfers and script execution.

Once connected, I confirmed our elevated privileges:
...
└─────────────────────────────────────────────────────────────────┘
    ...
    └────────┬─────────┘
             ...
             ▼
    ┌──────────────────┐     ┌─────────────────────────────────┐
    │ Network Recon    │────▶│ Identify SMB Signing Disabled   │
    │ (Nmap/NetExec)   │     │ on domain-joined systems        │
    └────────┬─────────┘     └─────────────────────────────────┘
             │
             ▼
    ┌──────────────────┐     ┌─────────────────────────────────┐
    │ Network Poison   │────▶│ Capture NTLMv2 hashes from      │
    │ (Responder+MITM6)│     │ broadcast traffic               │
    └────────┬─────────┘     └─────────────────────────────────┘
             │
             ▼
    ┌──────────────────┐     ┌─────────────────────────────────┐
    │ NTLM Relay       │────▶│ Relay auth to DC LDAP service   │
    │ (ntlmrelayx)     │     │ Create machine accounts         │
    └────────┬─────────┘     └─────────────────────────────────┘
             │
             ▼
    ┌──────────────────┐     ┌─────────────────────────────────┐
    │ ACL Manipulation │────▶│ Grant DCSync privileges         │
    │                  │     │ Add to Enterprise Admins        │
    └────────┬─────────┘     └─────────────────────────────────┘
             │
             ▼
    ┌──────────────────┐     ┌─────────────────────────────────┐
    │ DCSync Attack    │────▶│ Extract ALL domain credentials  │
    │ (secretsdump)    │     │ including krbtgt hash           │
    └────────┬─────────┘     └─────────────────────────────────┘
             │
             ▼
    ┌──────────────────┐     ┌─────────────────────────────────┐
    │ Domain Takeover  │────▶│ Full Enterprise Admin access    │
    │ (Evil-WinRM)     │     │ Complete forest compromise      │
    └──────────────────┘     └─────────────────────────────────┘
```
...
|------------------|--------|
| **SMB Signing Not Required** | Almost all machines in the domain had SMB signing disabled, allowing for NTLM relay attacks against services like LDAP |
...
3. **Disable NBT-NS**
   - Configure via DHCP options or network adapter settings

4. **Set Machine Account Quota to 0**
   ```powershell
   Set-ADDomain -Identity "corp.local" -Replace @{"ms-DS-MachineAccountQuota"="0"}
   ```

5. **Secure or Disable IPv6**
   - If IPv6 is not required, disable it via Group Policy
   - If required, implement proper IPv6 security controls

### Long-term Security Improvements

- Implement tiered administration model
- Deploy Protected Users security group for privileged accounts
- Enable Advanced Audit Policy for DCSync detection (Event ID 4662)
- Regular Active Directory security assessments
- Network segmentation between client and server VLANs
...
- [Impacket](https://github.com/fortra/impacket) - Network protocol toolkit
- [Responder](https://github.com/lgandx/Responder) - LLMNR/NBT-NS poisoner
- [MITM6](https://github.com/dirkjanm/mitm6) - IPv6 MITM tool
- [NetExec](https://github.com/Pennyw0rth/NetExec) - Network execution tool (CrackMapExec successor)
